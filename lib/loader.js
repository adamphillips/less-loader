...
function getWebpackFileManager(less, loaderContext, query, isSync) {
    ...
    WebpackFileManager.prototype.loadFile = function (filename, currentDirectory, options, environment, callback) {
        ...
        const fallbackRequest = filename;

        function loadModuleFromFilename(filename) {
        ...
        loaderContext.resolve(context, moduleRequest, (err, resolvedFilename) => {
            if (err) {
                loaderContext.resolve(fallbackContext, fallbackRequest, (err, resolvedFilename) => {
                    if (err) {
                        callback(err);
                        return;
                    }

                    loadModuleFromFilename(resolvedFilename);
                });
